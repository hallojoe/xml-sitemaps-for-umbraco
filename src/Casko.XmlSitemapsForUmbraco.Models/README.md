...
- This project does not serialize, deserialize, stream, return HTTP results, cache, or store XML. Serialization and HTTP result helpers belong in `Casko.XmlSitemapsForUmbraco.Common`; storage belongs in `Casko.XmlSitemapsForUmbraco.Storage`.
...
| Project | Usage |
|---|---|
| `../Casko.XmlSitemapsForUmbraco.Common/Casko.XmlSitemapsForUmbraco.Common.csproj` | Shares model contracts and provides XML serialization and HTTP result helpers. |
...
| `IXmlSitemapModel.cs` | Marker interface for sitemap model roots consumed by Common serializers and HTTP results. |
...
There is no dedicated test project for this library. Existing unit tests cover the models through Common serialization and HTTP-result helpers, rendering, storage, and provider tests in `../Casko.XmlSitemapsForUmbraco.Tests`.
