...
public class SitemapRenderingServiceTests
{
    ...
    public void Render_WhenRootHasDescendant_IncludesRootAndDescendant()
    {
        ...
        var contentCollector = Substitute.For<IXmlSitemapContentCollector>();
        contentCollector.Collect(Arg.Any<XmlSitemapRenderContext>()).Returns([root, child]);
        var urlRenderer = Substitute.For<IXmlSitemapUrlRenderer>();
        ...
        var sut = new XmlSitemapRenderer(contentCollector, urlRenderer);

        var result = sut.Render(new XmlSitemapRenderContext([root], "en", ["en"], "example.com"));
    ...
    public void Render_WhenDocumentTypeFilterExcludesContent_ExcludesDisallowedContent()
    {
        ...
        var result = sut.Render(new XmlSitemapRenderContext(
            [root],
            "en",
            ["en"],
            Hostname: null,
    ...
    public void Render_WhenFilterExcludesEveryContentItem_ThrowsRootContentHasNoContentException()
    {
        ...
        var contentCollector = Substitute.For<IXmlSitemapContentCollector>();
        contentCollector.Collect(Arg.Any<XmlSitemapRenderContext>()).Returns([root]);
        var sut = new XmlSitemapRenderer(contentCollector, Substitute.For<IXmlSitemapUrlRenderer>());

        TestDelegate action = () => sut.Render(new XmlSitemapRenderContext(
            [root],
            "en",
            ["en"],
            Hostname: null,
    ...
    public void Render_WhenUrlsHaveSameLocation_DeduplicatesCaseInsensitively()
    {
        ...
        var result = sut.Render(new XmlSitemapRenderContext([root], "en", ["en"], Hostname: null));
    ...
    public void RenderUrl_SetsLocationLastModifiedAndCultureLinks()
    {
        ...
        var urlBuilder = Substitute.For<ISitemapUrlBuilder>();
        urlBuilder.BuildContentUrl(content, "en", "example.com").Returns("https://example.com/en");
        var cultureLinkRenderer = Substitute.For<IXmlSitemapUrlCultureLinkRenderer>();
        ...
        var sut = new XmlSitemapUrlRenderer(urlBuilder, cultureLinkRenderer);
    ...
    public void RenderCultureLinks_PutsCurrentCultureFirstAndSkipsFragmentUrls()
    {
        ...
        var sut = new XmlSitemapUrlCultureLinkRenderer(urlBuilder);
