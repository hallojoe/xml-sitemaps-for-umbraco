...
XML Sitemaps for Umbraco helps Umbraco 17+ projects publish XML sitemaps. It is designed for teams that want reliable sitemap delivery for search engines without having to build and maintain the full solution themselves.

## What It Does

- Creates XML sitemaps from Umbraco content.
- Supports sitemap indexes for larger, multilingual, or multi-site solutions.
...
- Adds a read-only backoffice configuration overview for sitemaps, filters, storage, and indexes.
...
- Keeps sitemap output extensible when content comes from external systems.
- Packages the split internal implementation as one NuGet package for consuming projects.
...
- Multi-site solutions that need host-specific sitemap filenames.
...
The package is configured in `appsettings.json` under `XmlSitemaps` and can expose sitemap files directly on the site using rewrite URLs. Teams can define:
...
- Which custom providers should add project-specific sitemap output.
- How stored media files and background refreshes should behave.
...
- You want a package that can start simple and still support custom sitemap providers later.
